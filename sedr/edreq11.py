...
def requirementA2_2_A5(jsondata: json) -> (bool, str):
    ...
    for url in conformance_urls:
        if not url in jsondata:
            return False, f"Conformance page /conformance does not contain the core edr class {url}. See <{spec_url}> for more info."
...
def requirementA11_1(jsondata: json) -> (bool, str):
    ...
    for url in jsondata:
        if url in openapi_conformance_urls:
            hit = url
            if util.args.openapi_version == "3.1" and "oas31" in url or \
                util.args.openapi_version == "3.0" and "oas30" in url:
                    return True, url
            else:
                return False, f"OpenAPI version {util.args.openapi_version} and version in conformance {url} doesn't match. See <{spec_url}> for more info."
            return True, url
    else:
        return False, f"Conformance page /conformance does not contain an openapi class. See <{spec_url}> for more info."
